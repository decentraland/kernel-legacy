...
export function isOnLimits(
  { maximum, minimum }: BoundingInfo,
  parcels: Vector3[]
): boolean {
...
export function decodeParcelSceneBoundaries(boundaries: string) {
  const [base, ...parcels] = boundaries
    .split(/\s*;\s*/)
    .map($ => parseParcelPosition($));
  return { base, parcels };
}
...
function visitParcel(
  parcel: Vector2,
  allParcels: Vector2[] = [parcel],
  visited: Vector2[] = []
): Vector2[] {
  let isVisited = visited.some(visitedParcel =>
    isEqual2(visitedParcel, parcel)
  );
  if (!isVisited) {
    visited.push(parcel);
