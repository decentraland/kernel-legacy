...
function resolveBazel(
  ...
  function resolveInRootDir(importee) {
    ...
    if (DEBUG)
      console.error(`Rollup: try to resolve '${importee}' at '${candidate}'`)
    try {
  ...
  if (!resolved) {
    ...
    for (const k in moduleMappings) {
      if (normalizedImportee == k || normalizedImportee.startsWith(k + '/')) {
        ...
        const mappedImportee = path.join(
          v,
          normalizedImportee.slice(k.length + 1)
        )
        if (DEBUG)
          console.error(
            `Rollup: module mapped '${importee}' to '${mappedImportee}'`
          )
  ...
  if (DEBUG) {
    if (resolved) {
      ...
      console.error(
        `Rollup: allowing rollup to resolve '${importee}' with node module resolution`
      )
    }
  }
...
const config = {
  ...
  plugins: [TMPL_additional_plugins].concat([
    ...
    commonjs({
      include: [
        'node_modules/**',
        path.join(process.cwd(), '..', '..', 'npm', 'node_modules') + '/**'
      ],
