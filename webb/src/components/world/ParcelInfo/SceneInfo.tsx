...
export class SceneInfo extends React.Component<any> {
  render() {
    ...
    const loading =
      !this.props.parcelData[coordinate] ||
    ...
    return (
      <Segment>
        <HeaderMenu>
          <HeaderMenu.Left>
            <Header>
              Scene: <Address value={scene} />
            </Header>
          </HeaderMenu.Left>
          <HeaderMenu.Right>
            <Header>
              <Button onClick={() => this.props.push(sceneUrl(scene))} basic>
                Start scene <Icon name='chevron right' />
              </Button>
            </Header>
          </HeaderMenu.Right>
        </HeaderMenu>
  ...
  render() {
    ...
    return (
      <>
        ...
        {tabs
          .filter(t => this.state.tab === t.name)
          .map(({ name, data }) => (
            <pre key={name + '-'}>{JSON.stringify(data, null, 2)}</pre>
          ))}
      </>
    )
  }
