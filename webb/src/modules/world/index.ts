  ...
  switch (action.type) {
    ...
    case 'Set coordinate to scene data':
      return {
        ...state,
        coordinateToScene: {
          ...state.coordinateToScene,
          [`${action.payload.x}.${action.payload.y}`]: action.payload.scene
        }
      }
    ...
    case 'Loading land api data':
      return {
        ...
        parcelData: {
          ...state.parcelData,
          [`${action.payload.x}.${action.payload.y}`]: { loading: true }
        }
      }
    case 'Loading parcel to scene mapping':
      return {
        ...
        coordinateToScene: {
          ...state.coordinateToScene,
          [`${action.payload.parcel.x}.${action.payload.parcel.y}`]: {
            loading: true
          }
        }
      }
    ...
    case 'Loading scene file mappings':
      return {
        ...
        mappings: {
          ...state.mappings,
          [action.payload.scene]: { loading: true }
        }
      }
  ...
  switch (action.type) {
    ...
    case 'Set coordinate to scene data':
      ...
      if (!store.getState().world.mappings[sceneData]) {
        store.dispatch({
          type: 'Loading scene file mappings',
          payload: action.payload
        })
...
export async function fetchParcelToSceneMapping(
  ...
  for (let value of apiData.data) {
    ...
    dispatch({
      type: 'Set coordinate to scene data',
      payload: { x: dx, y: dy, scene: thisScene }
    })
  }
  if (!thisScene) {
...
export async function fetchSceneMappings(
  store: Store<WorldRootState>,
  payload: { x: number; y: number; scene: string }
) {
  const dispatch = (type: any, payload?: any) =>
