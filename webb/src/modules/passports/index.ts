...
export type PassportState = {
  info: { [key: string]: { status: 'loading'|'error', data: any } | { status: 'ok', data: StoredPassport } }
}
...
export function passportsReducer(state?: PassportState, action?: AnyAction): PassportState {
  ...
  switch (action.type) {
    case 'Fetching passport':
      return { ...state, info: {...state.info, [action.payload]: { status: 'loading' } } }
    case 'Profile passport':
      return { ...state, info: {...state.info, [action.payload.userId]: {status: 'ok', data: action.payload } } }
    case 'Error fetching passport':
      return { ...state, info: {...state.info, [action.payload.userId]: { status: 'error', data: action.payload } } }
    default:
      return state
  }
}
 ...
 */
export const passportsMiddleware = (store: Store<PassportRootState & AuthRootState>) => (next: Middleware) => (action: any) => {
