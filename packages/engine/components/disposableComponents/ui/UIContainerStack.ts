...
import { UIContainerStackShape, UIStackType } from 'decentraland-ecs/src/decentraland/UIShapes'
...
const schemaValidator = createSchemaValidator({
  ...
  stackType: { type: 'UIStackType', default: UIStackType.VERTICAL }
})
...
export class UIContainerStack extends UIControl<UIContainerStackShape, BABYLON.GUI.StackPanel> {
  ...
  async updateData(data: UIContainerStackShape): Promise<void> {
    ...
    this.control.isVertical = this.data.stackType == UIStackType.VERTICAL

    // Only set height manually if the stack container is not in vertical mode
    if (this.data.stackType != UIStackType.VERTICAL) {
