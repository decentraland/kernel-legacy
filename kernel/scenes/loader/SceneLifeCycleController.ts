...
import { SceneLifeCycleStatus, SceneLifeCycleStatusType } from '../types/SceneLifeCycleStatus'
import { Observable, Vector2 } from '@dcl/utils'
...
export class SceneLifeCycleController extends EventEmitter {
  ...
  contains(status: SceneLifeCycleStatus, position: Vector2) {
    return (
    ...
    if (!this.futureOfPositionToSceneId.has(position)) {
      ...
      try {
        ...
        for (const pos of land.scene.scene.parcels) {
          ...
          if (futurePosition !== undefined && futurePosition.isPending) {
